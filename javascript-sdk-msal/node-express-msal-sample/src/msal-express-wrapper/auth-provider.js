const { InteractionRequiredAuthError } = require("@azure/msal-common");
...
class AuthProvider {
        ...
        case constants.AppStages.SIGN_IN: {
          ...
          try {
            ...
            if (
              ...
              return res
                .status(200)
                .redirect(this.appSettings.settings.homePageRoute);
            } else {
  ...
  getToken = async (req, res, next) => {
    ...
    const scopes = Object.values(this.appSettings.resources).find(
      (resource) => resource.callingPageRoute === req.route.path
    ).scopes;
  ...
  getResourceName = (path) => {
    const index = Object.values(this.appSettings.resources).findIndex(
      (resource) => resource.callingPageRoute === path
    );
