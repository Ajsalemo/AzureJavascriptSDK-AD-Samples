...
exports.getProfilePage = async (req, res, next) => {
  ...
  try {
    profile = await fetchManager.callAPI(
      req.app.locals.appSettings.resources.graphAPI.endpoint,
...
exports.getTenantPage = async (req, res, next) => {
  ...
  try {
    ...
    // permission
    tenant = await fetchManager.callAPI(
      req.app.locals.appSettings.resources.armAPI.endpoint,
      req.session["armAPI"].accessToken
    );
  } catch (error) {
    console.log(error);
  }
