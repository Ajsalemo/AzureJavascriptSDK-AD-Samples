...
const { AuthorityStrings } = require("./constants");
...
class ConfigurationUtils {
   ...
   */
  static validateAppSettings = (config) => {
    if (
   ...
   */
  static getMsalConfiguration = (config, cachePlugin = null) => {
    return {
      auth: {
        clientId: config.credentials.clientId,
        authority: config.policies
          ? config.policies.signUpSignIn.authority
          : AuthorityStrings.AAD + config.credentials.tenantId, // single tenant
        clientSecret: config.credentials.clientSecret,
        redirectUri: config.settings ? config.settings.redirectUri : "", // defaults to calling page
        knownAuthorities: config.policies
          ? [config.policies.authorityDomain]
          : [], // in B2C scenarios
      },
      cache: {
        cachePlugin,
