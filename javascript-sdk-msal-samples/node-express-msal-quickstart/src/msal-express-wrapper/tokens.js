...
class TokenValidator {
  ...
  validateIdToken = (idTokenClaims) => {
    ...
    const checkTenant =
      (this.appSettings.policies && !idTokenClaims["tid"]) ||
      idTokenClaims["tid"] === this.appSettings.credentials.tenantId
        ? true
        : false;
   ...
   */
  validateAccessToken = async (accessToken, protectedRoute) => {
    const now = Math.round(new Date().getTime() / 1000); // in UNIX format
